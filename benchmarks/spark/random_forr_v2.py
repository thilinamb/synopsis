...
def main():
    ...
    # load time test
	input_list = ('test/southeast-0*-*.txt', 'sample/sample-10.txt', 'sample/sample-20.txt', 'syn/southeast-synth-1.txt', '/syn/southeast-synth-2.txt', '/syn-100/southeast-0*-*.txt')
    #intput_list = ('/syn-100/southeast-0*-*.txt')
	load_times = []        
	for i in range(6):
		load_time_i = []
	...
	syn100 = sc.textFile(hdfs_nn + '/syn-100/southeast-0*-*.txt')
	syn100_json=syn100.map(load_json).filter(lambda x: len(x) > 0)
	syn100_labeled = syn100_json.map(parse_json_record)
	print('Synthetic 100% Parition Count: ', syn100_labeled.getNumPartitions())
	...
	print('Record Counts - Syn 20%: ', syn100_labeled.count())
	...
	for i in range(iteration_count):
    	...
    	# syn - 20%
		syn100_t_start = time.time()
		syn100_model = RandomForest.trainRegressor(syn100_labeled,
                                    categoricalFeaturesInfo={}, maxDepth=20, 
                                    numTrees=32, maxBins=32)
		syn100_t_stop = time.time()		
    	...
    	# syn - 20%
		syn100_predictions = syn100_model.predict(test_rdd.map(lambda x: x.features))
		syn100_labelsAndPredictions = test_rdd.map(lambda lp: lp.label).zip(syn100_predictions)
		syn100_testMSE = syn100_labelsAndPredictions.map(lambda v: (v[0] - v[1]) * (v[0] - v[1])).sum() / float(test_rdd.count())
