...
The objective of scaling out is to maintain the \emph{stability} of each sketchlet.
...
As the data flow tree grows with scaling out operations, having parent nodes pass traffic through to their children becomes inefficient because of higher bandwidth consumption as well as increased latency due to additional network hops the packets have to travel through.
To circumvent this, we allow \emph{short circuiting}, which directs traffic from stream ingesters straight to child nodes.
For instance, stream ingesters will send data directly to node D using the short circuited route bypassing nodes A and C in Figure~\ref{fig:dist-sketch}. 
